 ...
 * @author ilyes
 */
public class AddTransporteurForm extends BaseForm{

    
    Form current;
    public AddTransporteurForm(Resources res ) {
          super("Transporteurs",BoxLayout.y()); //herigate men Newsfeed w l formulaire vertical
        Toolbar tb = new Toolbar(true);
        current = this ;
        setToolbar(tb);
        getTitleArea().setUIID("Container");
        setTitle("Ajouter Transporteur");
...
TextField numTelField = new TextField("", "Num Tel");

Style textFieldStyle = new Style();
textFieldStyle.setFgColor(0x000000);

idField.setUnselectedStyle(textFieldStyle);
...
String numTel = numTelField.getText().trim();
    ...
    transporteur.setNumTel(Integer.parseInt(numTel));

    ServiceTransporteur.getInstance().addTransporteur(transporteur);
    new ListTransporteurForm(res).show();
} catch (NumberFormatException ex) {
    Dialog.show("Error", "Please enter valid integer value for Num Tel", "OK", null);
}
});
